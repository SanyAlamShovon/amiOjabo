...
const byId = {
  async: async function (request, reply) {
    try {
      const data =  await driverModel.find({_id : request.params._id});
      if(data === null || data === undefined) reply({}).code(404);
      else  reply(data).code(200);
    } catch (err) {
      reply(Boom.badRequest(err.toString())).code(400);
...
const update = {
  async : async function(request,reply){
    try{
     const data =  await driverModel.update({_id : request.params._id},request.payload);
     if(data === null || data === undefined)reply({}).code(404);
     else reply({}).code(204)
    }catch(err){
      reply(Boom.badRequest(err.toString())),code(400);
...
const destroy = {
  async : async function(request,reply){
    try{
      const data = await driverModel.remove({_id : request.params._id});
      if(data == null || data === undefined)reply({}).code(404);
      else reply({}).code(200);
    }catch(err){
      reply(Boom.badRequest(err.toString())).code(400);
